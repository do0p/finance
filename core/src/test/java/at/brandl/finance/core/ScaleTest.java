...
public class ScaleTest {
	...
	public void scaleReal() throws IOException {
		Data data = scale.scale(new RewindableFileReader(DATA_FILENAME), new BufferedWriter(new FileWriter(SAVE_FILENAME)), null, -5, 5);
	...
	public void scale() throws IOException {
		...
		Data data1 = scale.scale(dataFile1, saveFile, null, -1, 1);
		...
		// saveFile
		BufferedReader saveFileReader = new BufferedReader(new StringReader(saveStringWriter.toString()));
		...
		Assert.assertEquals("-1.000000000000000 1.000000000000000", readAndCopy(saveFileReader, restoreFile));
		Assert.assertEquals("1 -50.10000000000000 -3.500000000000000", readAndCopy(saveFileReader, restoreFile));
		Assert.assertEquals("2 0.02000000000000000 1.800000000000000", readAndCopy(saveFileReader, restoreFile));
		Assert.assertEquals("5 0.000000000000000 1.000000000000000", readAndCopy(saveFileReader, restoreFile));
		Assert.assertEquals("6 0.000000000000000 1.000000000000000", readAndCopy(saveFileReader, restoreFile));
		Assert.assertEquals("7 0.000000000000000 1.000000000000000", readAndCopy(saveFileReader, restoreFile));
		Assert.assertEquals("10 0.000000000000000 1.000000000000000", readAndCopy(saveFileReader, restoreFile));
		
		// restore
		RewindableReader dataFile2 = createDataFile2();
		Data data2 = scale.scale(dataFile2, null, restoreFile, 0, 2);
